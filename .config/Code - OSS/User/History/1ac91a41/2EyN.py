...
def create_blog_post(title, short_description, tags):
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                # print("query")
                insert_query = '''
                    INSERT INTO blogs (title, short_description, tags)
                    VALUES (%s, %s, %s, %s)
                    RETURNING *;
                '''
                cursor.execute(insert_query, (title, short_description, tags))
